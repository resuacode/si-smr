...
:::danger ⚠️ IMPORTANTE: Crear Snapshots o Clones Enlazados ANTES de Comenzar
...
1. **Apaga completamente la máquina virtual** (no la suspendas)
2. Clic derecho sobre la VM en VirtualBox → **"Clonar..."**
3. **Nombre**: `[Nombre-Original]-LAB-Original` (ej: `SAD-Ubuntu-Server-LAB-Original`)
4. **Tipo de clonación**: Selecciona **"Clonación enlazada"**
5. **Instantánea**: Mantén "Estado actual de la máquina"
6. Finaliza el proceso

### Opción B: Crear Snapshots

1. Selecciona la VM en VirtualBox
2. Ve a la pestaña **"Instantáneas"** 
3. Clic en **"Tomar"** (icono de cámara)
4. **Nombre**: `Estado-Inicial-Ejercicios`
5. **Descripción**: `Snapshot antes de ejercicios de control de acceso lógico`
...
**Enunciado:**
1. Accede al servidor Ubuntu Server vía SSH
2. Configura una política de contraseñas que requiera:
...
3. Configura el bloqueo de cuentas después de 3 intentos fallidos durante 10 minutos
4. Crea tres usuarios de prueba: `user1`, `user2`, `user3`
5. Prueba la política intentando establecer contraseñas débiles
...
**Enunciado:**
1. Modifica la configuración SSH para:
   - Cambiar el puerto por defecto a 2222
   - Deshabilitar login de root
   - Permitir solo autenticación por clave pública para el usuario `admin`
   - Limitar intentos de login a 3 por conexión
...
5. Desde Kali Security, intenta conectarte con diferentes métodos y analiza los logs
...
1. En Ubuntu Server:
   ...
   - Extrae el archivo `/etc/shadow` para análisis
2. En Kali Security:
...
3. Analiza los resultados y propón mejoras en la política de contraseñas
...
**Enunciado:**
1. Configura Samba con las siguientes características:
   - Compartición `publico/` con acceso de solo lectura para todos los usuarios autenticados
   - Compartición `privado/` con acceso completo solo para usuarios del grupo `backup`
   - Compartición `departamentos/` con subdirectorios que tengan acceso específico por grupos
...
1. Configura diferentes niveles de acceso sudo:
   - Grupo `operators`: puede reiniciar servicios específicos (apache2, mysql, ssh)
   - Grupo `backup_admins`: puede ejecutar comandos de backup y gestión de archivos
   - Grupo `network_admins`: puede ejecutar comandos de red y firewall
2. Crea usuarios y asígnalos a estos grupos
...
1. En Ubuntu Server:
   ...
   - Implementa monitorización de archivos críticos con `inotify`
   - Configura logs centralizados para servicios críticos
...
**Enunciado:**
1. Accede al Windows Server vía RDP (localhost:3389)
   - Usuario: `vagrant` / Contraseña: `vagrant`
   - O utiliza: `labadmin` / Contraseña: `Password123!`
2. Configura las siguientes políticas de seguridad local usando `secpol.msc`:
...
3. Asigna usuarios a grupos:
   - `gerente` → `Admin_Group`
   - `empleado1` → `Contab_Group`
   - `empleado2` → `Dev_Group`
   - Crea `empleado3` y asígnalo a `RRHH_Group`
4. Configura permisos NTFS (clic derecho → Propiedades → Seguridad):
...
6. Desde Ubuntu Server, prueba el acceso a los recursos compartidos:
   ...
   ```
7. Habilita auditoría de acceso a archivos:
...
1. Asegúrate de que los ejercicios 8 y 9 estén completados (usuarios, grupos, recursos compartidos)
2. Crea un usuario adicional específico para este ejercicio:
   - Usuario: `empleado_remoto`
   - Contraseña: `Temporal123!` (que deberá cambiar en el primer inicio)
   - Grupo: `Dev_Group`
3. Configura una política adicional:
...
1. Accede al Windows Client vía RDP (localhost:3390)
2. Intenta conectarte a los recursos compartidos del servidor usando diferentes métodos:
...
3. **Pruebas de políticas de seguridad:**
...
4. **Monitorización desde el cliente:**
   - Abre el Visor de eventos local (`eventvwr.msc`)
   - Revisa los eventos de seguridad relacionados con autenticación de red
...
2. **Herramientas de diagnóstico Windows:**
   ```cmd
...
3. **Simulación de problemas comunes:**
...
1. Acciones de contención:
   - Fuerza el cambio de contraseña del usuario `admin`
...
2. Investigación:
   ...
   - Revisa `/var/log/secure` para intentos de escalación de privilegios
   - Examina el historial de comandos de usuarios comprometidos
   - Verifica integridad de archivos críticos con `aide` o checksums

**Parte B: Respuesta inmediata en Windows (Windows Server)**
1. Acciones de contención:
   - Deshabilita temporalmente las cuentas comprometidas
   - Fuerza el cambio de contraseña de `labadmin` y otros usuarios administrativos
...
1. **Linux - Medidas adicionales:**
   ...
   - Habilita auditoría avanzada con `auditd`
   - Implementa monitorización de archivos críticos con `inotify`
   - Configura alertas automáticas por email/syslog
...
1. **Configuraciones realizadas**: Archivos modificados y cambios específicos
2. **Pruebas ejecutadas**: Capturas de pantalla de las pruebas de funcionamiento
3. **Resultados obtenidos**: Verificación de que los objetivos se cumplieron
4. **Problemas encontrados**: Dificultades y cómo se resolvieron
5. **Mejoras propuestas**: Sugerencias para fortalecer aún más la seguridad
...
- Utiliza `man` para obtener información detallada de los comandos utilizados
- Documenta todos los cambios para poder revertirlos si es necesario
:::

### Formato de la entrega

Por supuesto, la entrega debe ser individual y en formato PDF, respetando todo el formato establecido en la [**sección de formato de entregas**](../formato-entregas.mdx).
...
- Nombre del archivo: `SI_Tema2_NombreApellido.pdf`
- Fecha límite de entrega: *Consultar en el Aula Virtual*.

:::warning ⚠️ Sobre el número de páginas de esta entrega
